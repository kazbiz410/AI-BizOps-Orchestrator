...
class GenerationService:
    ...
    def generate_recommendation_content(self, organization_id: str, force_refresh: bool = False) -> dict:
        if not force_refresh:
            saved_content = self.repository.get_generated_recommendation_content(organization_id)
            if saved_content:
                return self._restore_generated_recommendation_content(saved_content)
    ...
    def generate_blueprints(self, organization_id: str, force_refresh: bool = False) -> dict:
        if not force_refresh:
            saved_blueprints = self.repository.list_blueprints(organization_id)
            saved_tasks = self.repository.list_implementation_tasks(organization_id)
            if saved_blueprints and saved_tasks:
                return {
    ...
    def generate_n8n_draft(self, organization_id: str, force_refresh: bool = False) -> dict:
        if not force_refresh:
            saved_draft = self.repository.get_generated_n8n_draft(organization_id)
            if saved_draft:
                return self._restore_generated_n8n_draft(organization_id, saved_draft)
