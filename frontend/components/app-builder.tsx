...
export function AppBuilder() {
  ...
  async function handleGenerateRecommendations(organizationId: string) {
    await runAction(`recommend-${organizationId}`, async () => {
      try {
        ...
        setStatus("Recommendation Agent が提案文とROI仮説をまとめました。");
      } catch (error) {
        setStatus(
  ...
  async function handleGenerateBlueprints(organizationId: string) {
    await runAction(`blueprint-${organizationId}`, async () => {
      try {
        ...
        setStatus("Blueprint Agent が図と実装タスクを生成しました。");
      } catch (error) {
        setStatus(
  ...
  async function handleGenerateN8nDraft(organizationId: string) {
    await runAction(`n8n-${organizationId}`, async () => {
      try {
        ...
        setStatus("Blueprint Agent が n8n draft を生成しました。");
      } catch (error) {
        setStatus(
  ...
  async function handleGenerateQuestions(organizationId: string) {
    await runAction(`question-${organizationId}`, async () => {
      try {
        ...
        setStatus("Interview Agent が不足情報の質問文を生成しました。");
      } catch (error) {
        setStatus(
            ...
            {selectedOrganization ? (
              <>
                ...
                <button
                  ...
                  disabled={Boolean(activeAction)}
                >
                  {isGeneratingRecommendation ? "生成中..." : "提案文を生成する"}
                </button>
                <button
                  type="button"
                  ...
                  disabled={Boolean(activeAction)}
                >
                  {isGeneratingBlueprint ? "図を生成中..." : "図と実装タスクを生成する"}
                </button>
                <button
                  type="button"
                  ...
                  disabled={Boolean(activeAction)}
                >
                  {isGeneratingN8n ? "生成中..." : "n8nドラフトを生成する"}
                </button>
                <button
                  type="button"
                  ...
                  disabled={Boolean(activeAction)}
                >
                  {isGeneratingQuestion ? "生成中..." : "Slack質問を生成する"}
                </button>
              </>
            ) : (
